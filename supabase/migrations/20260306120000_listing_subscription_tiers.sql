-- Hybrid listing subscription tiers:
-- - Listing-level tier override with account-plan fallback
-- - Top-tier feature scaffold JSON for future capabilities
-- - Public view projections for effective tier and card size

ALTER TABLE listings
  ADD COLUMN IF NOT EXISTS subscription_tier_override text,
  ADD COLUMN IF NOT EXISTS top_tier_features jsonb NOT NULL DEFAULT '{}'::jsonb;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'listings_subscription_tier_override_check'
  ) THEN
    ALTER TABLE listings
      ADD CONSTRAINT listings_subscription_tier_override_check
      CHECK (
        subscription_tier_override IS NULL
        OR subscription_tier_override IN ('base', 'middle', 'top')
      );
  END IF;
END
$$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'listings_top_tier_features_is_object_check'
  ) THEN
    ALTER TABLE listings
      ADD CONSTRAINT listings_top_tier_features_is_object_check
      CHECK (jsonb_typeof(top_tier_features) = 'object');
  END IF;
END
$$;

COMMENT ON COLUMN listings.subscription_tier_override IS
  'Optional per-listing tier override. Null derives from tenant/account plan.';

COMMENT ON COLUMN listings.top_tier_features IS
  'Scaffold for top-tier feature flags and metadata.';
...
COMMENT ON VIEW public_listings_view IS
  'Public listings view with image ordering, claim visibility, and effective subscription tiers.';
