...
export default function ZoomCallbackPage() {
  ...
  useEffect(() => {
    ...
    const handleCallback = async () => {
      try {
        const response = await fetch("/api/integrations/video/zoom/callback", {
          method: "POST",
        ...
        if (!response.ok) {
          ...
          throw new Error(error.message || "Failed to connect Zoom");
        }
        ...
        setStatus("error");
        setMessage(error.message || "Failed to connect Zoom account");
        setTimeout(() => router.push("/bookings/integrations"), 3000);
      }
    };
  ...
  return (
    <div>
      <PageBreadcrumb pageTitle="Connecting Zoom" />
