...
export default function BookingDetailPage() {
  ...
  const loadBooking = async () => {
    try {
      ...
      // Load video meeting info if available
      if (data.video_meeting_url) {
        setVideoMeeting({
          url: data.video_meeting_url,
          provider: data.video_meeting_provider,
          password: data.video_meeting_password,
        });
      }
      
      // Load team member info if assigned
      if (data.team_member_id) {
        // TODO: Fetch team member details
        setTeamMember({ id: data.team_member_id });
      }
    } catch (error) {
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* Video Meeting */}
            {videoMeeting && (
              <div className="rounded-2xl border border-purple-200 bg-purple-50 p-6 shadow-sm dark:border-purple-800 dark:bg-purple-900/20">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <span className="ml-auto inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium bg-purple-100 text-purple-700 dark:bg-purple-500/15 dark:text-purple-500 capitalize">
                    {videoMeeting.provider}
                  </span>
                </div>
                <div className="space-y-3">
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <a
                        href={videoMeeting.url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center gap-2 text-purple-600 dark:text-purple-400 hover:underline break-all"
                      >
                        <LinkIcon className="h-4 w-4 flex-shrink-0" />
                        <span className="truncate">{videoMeeting.url}</span>
                      </a>
                    </div>
                  </div>
                  {videoMeeting.password && (
                    <div>
                      ...
                      <p className="font-mono text-sm text-purple-900 dark:text-purple-400">
                        {videoMeeting.password}
                      </p>
                    </div>
                  )}
          ...
          <div className="space-y-6">
              ...
              <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    Type: {eventType.booking_type} | Video: {eventType.video_provider || "none"}
                  </p>
                </div>
              </div>
            )}
            ...
            {teamMember && (
              <div className="rounded-2xl border border-blue-200 bg-blue-50 p-6 shadow-sm dark:border-blue-800 dark:bg-blue-900/20">
                <div className="flex items-center gap-3 mb-4">
                ...
                <div className="space-y-2">
                  <p className="text-sm text-blue-700 dark:text-blue-300">
                    This booking was assigned via round robin scheduling.
                  </p>
                  {teamMember.round_robin_enabled && (
