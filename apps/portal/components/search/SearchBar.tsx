...
export function SearchBar({
  ...
  const gridCols = showFiltersButton
  ...
  return (
    <form
      ...
      className={`grid gap-x-2 gap-y-3 ${gridCols} ${className}`}
    >
      {/* Row 1: Service type, search input, search button, optional filters */}
      <select
        value={serviceType}
      ...
      {/* Row 2: Date, location input, Use My Location — same column widths as row 1 */}
      <label className="flex items-center gap-2 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
        ...
        <input
          ...
          className="bg-transparent border-0 outline-none text-gray-900 dark:text-white font-medium cursor-pointer min-w-0"
        />
      </label>
      ...
      <div className="relative min-w-0">
        ...
        <input
          ...
          className="block w-full pl-10 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all text-sm min-w-0"
        />
      </div>
      ...
      {showFiltersButton && <div aria-hidden="true" />}

      {locationError && (
        <p className="text-sm text-amber-600 dark:text-amber-400 col-span-full" role="alert">
          {locationError}
        </p>
      )}
    </form>
